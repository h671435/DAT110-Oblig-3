...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for (int i = 0; i < numReplicas; i++) {
			...
			String filnavn = filename + i;
			// hash the replica
			BigInteger filHash = Hash.hashOf(filnavn);

			// store the hash in the replicafiles array.
			replicafiles[i] = filHash;
		}
	}
    ...
    public int distributeReplicastoPeers() throws RemoteException {
        ...
        for (BigInteger replicafile : replicafiles) {
            // for each replica, find its successor (peer/node) by performing cessor(replica)
            NodeInterface succy = chordnode.findSuccessor(replicafile);
            // call the addKey on the successor and add the replica
            succy.addKey(replicafile);
            ...
            succy.saveFileContent(filename, replicafile, bytesOfFile, counter == index);
            counter++;
        }
	...
	public NodeInterface findPrimaryOfItem() {
		...
		for (Message message : activeNodesforFile) {
			...
			if (message.isPrimaryServer()) {
				primary = Util.getProcessStub(message.getNodeName(), message.getPort());
				return primary;
			}
		}
