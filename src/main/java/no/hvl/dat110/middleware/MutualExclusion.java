...
public class MutualExclusion {
    ...
    public void onMutexRequestReceived(Message message) throws RemoteException {
        ...
        if (!WANTS_TO_ENTER_CS && !CS_BUSY) {
            ...
            if (clock.getClock() > message.getClock()) {
                ...
                comparisonResult = node.getNodeName().compareTo(message.getNodeName());
            }
        }
        ...
        switch (condition) {
            ...
            case 2: {
                ...
                if (messageClock == ownClock) {
                    ...
                    int comparisonResult = node.getNodeName().compareTo(message.getNodeName());
                    if (comparisonResult < 0) {
                        // sender wins, acknowledge the message, obtain a stub and call onMutexAcknowledgementReceived()
                        message.setAcknowledged(true);
                        NodeInterface stub = Util.getProcessStub(message.getNodeName(), message.getPort());
    ...
    public void multicastReleaseLocks(Set<Message> activenodes) {
        ...
        for (Message an : activenodes) {
            // obtain a stub for each node from the registry
            NodeInterface nodeStub = Util.getProcessStub(an.getNodeName(), an.getPort());
            // call releaseLocks()
            try {
